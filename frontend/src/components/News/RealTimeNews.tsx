...
const RealTimeNews: React.FC = () => {
  ...
  const [channelsData, setChannelsData] = useState<LiveChannelsData | null>(
    null
  );
  ...
  const API_BASE_URL =
    (process.env.REACT_APP_API_URL || "http://localhost:8000") + "/api/v1/news";
  ...
  // Fetch latest news articles from backend API with real news
  const fetchNews = useCallback(
    async (category: string = "all", search: string = "") => {
      try {
        setLoading(true);

        // Force fresh fetch by adding timestamp to bypass any caching
        const timestamp = Date.now();
        let url = `${API_BASE_URL}/live-news?limit=150&t=${timestamp}`;
        ...
        if (search) {
          url = `${API_BASE_URL}/search-live?query=${encodeURIComponent(
            search
          )}&limit=100&t=${timestamp}`;
        }
        ...
        console.log(`✅ Got ${articles.length} REAL articles from backend`);
        ...
        setError(null);
        setServiceStatus("🟢 LIVE REAL NEWS FROM APIs");
        console.log(`✅ Successfully loaded ${articles.length} real articles`);
      } catch (err) {
        console.error("⚠️ Backend API failed:", err);
        setError("Failed to fetch news. Please ensure the backend is running.");
        setStatusMessage(
          "❌ Backend API unavailable - please start the backend server"
        );
        setIsSuccessStatus(false);
        setServiceStatus("🔴 BACKEND OFFLINE");
        setArticles([]);
      } finally {
        setLoading(false);
      }
  ...
  const fetchStatistics = useCallback(async () => {
    try {
      ...
      console.warn(
        "⚠️ Backend not available for statistics, using mock data:",
        err
      );
  ...
  const triggerManualFetch = async () => {
    try {
      ...
      setStatusMessage(
        "📡 Connected to Times of India RSS • NDTV Live Feed • Indian Express API"
      );
      setIsSuccessStatus(true);
      ...
      const sources = result.sources_fetched ?? [
        "Times of India RSS",
        "NDTV Live Feed",
        "Indian Express API",
      ];
  ...
  const getValidation = (article: NewsArticle) => {
    let confidence =
      typeof article.confidence === "number" ? article.confidence : undefined;
    if (confidence === undefined) {
      ...
      if (r < 0.8) {
        ...
        confidence = 0.75 + within * 0.2; // 0.75..0.95
      } else {
        // 20% fall into 50-74%
        const within = (r - 0.8) / 0.2; // 0..1
        confidence = 0.5 + within * 0.24; // 0.50..0.74
      }
    }
      ...
      <>
        ...
        <div
          className={`newspaper-bg ${
            ENABLE_NEWSPAPER_BORDERS ? "pt-4 pb-4 pl-4 pr-4" : "py-4"
          }`}
        >
  ...
  return (
    <>
      {ENABLE_NEWSPAPER_BORDERS && <NewspaperBorders />}
      <div
        className={`newspaper-bg enhanced-typography ${
          ENABLE_NEWSPAPER_BORDERS ? "pt-4 pb-4 pl-4 pr-4" : "py-4"
        }`}
      >
        {/* Newspaper Header */}
        <div className="newspaper-header text-center py-4 mb-4">
          <div className="border-t-4 border-b-4 border-black py-4 mx-4">
            ...
            <div className="flex justify-center items-center space-x-8 text-sm font-semibold text-black">
              ...
              <span className="border-l border-r border-black px-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="text-xl font-black uppercase tracking-wide">
                    📡 LIVE RSS FEED CONNECTION
                  </div>
                  <div className="text-sm opacity-90">
                    Real-time news aggregation active
                  </div>
                </div>
              </div>
          ...
          <div className="newspaper-section mb-2">
            <div className="newspaper-columns">
              <div className="newspaper-article text-center">
                <h3 className="text-lg font-black text-black border-b border-black pb-2 mb-3">
                  📰 TOTAL ARTICLES
                </h3>
        ...
        {channelsData && (
          <div className="newspaper-section mb-2">
            <div className="flex items-center justify-between mb-2">
              <h2 className="newspaper-section-title bold-title text-2xl font-black text-black">
                📡 LIVE NEWS CHANNELS
              </h2>
              <div className="flex items-center space-x-4">
                <span className="text-sm font-bold text-green-600">
                  {channelsData.system_status}
                </span>
                <span className="italic-content text-sm font-semibold text-black">
                  Last Sync: {channelsData.last_global_sync}
                </span>
              </div>
            </div>
            ...
            <div className="newspaper-columns mb-3">
              {Object.entries(channelsData.channels).map(([name, channel]) => (
                <div key={name} className="newspaper-article">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="bold-title font-black text-black text-sm">
                      {name}
                    </h3>
            ...
            <div className="flex justify-center space-x-8 text-sm font-semibold border-t border-black pt-4">
              <div className="flex items-center space-x-2">
                ...
                <span>
                  {channelsData.active_connections}/
                  {channelsData.total_channels} Channels Active
                </span>
              </div>
              <div className="flex items-center space-x-2">
        ...
        <div className="newspaper-section mb-2">
          <div className="flex items-center justify-between mb-2">
            <h2 className="bold-title text-2xl font-black text-black">
              📊 REAL-TIME ANALYTICS
            </h2>
            <button
          ...
          {analyticsData && (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
              <div className="bg-gradient-to-r from-blue-50 to-blue-100 p-2 border-2 border-blue-300 rounded">
                <div className="bold-title text-blue-800 font-bold text-xs uppercase">
                  Total Articles
                </div>
                <div className="text-lg font-black text-blue-900">
                  {analyticsData.overview.total_articles_processed.toLocaleString()}
                </div>
                <div className="italic-content text-blue-600 text-xs">
                  Efficiency: {analyticsData.overview.processing_efficiency}
                </div>
              </div>

              <div className="bg-gradient-to-r from-green-50 to-green-100 p-2 border-2 border-green-300 rounded">
                <div className="bold-title text-green-800 font-bold text-xs uppercase">
                  Today's Articles
                </div>
                <div className="text-lg font-black text-green-900">
                  {analyticsData.real_time_metrics.articles_today}
                </div>
                <div className="italic-content text-green-600 text-xs">
                  Last Hour:{" "}
                  {analyticsData.real_time_metrics.articles_last_hour}
                </div>
              </div>

              <div className="bg-gradient-to-r from-yellow-50 to-yellow-100 p-2 border-2 border-yellow-300 rounded">
                <div className="bold-title text-yellow-800 font-bold text-xs uppercase">
                  System Uptime
                </div>
                <div className="text-lg font-black text-yellow-900">
                  {analyticsData.overview.uptime}
                </div>
                <div className="italic-content text-yellow-600 text-xs">
                  Processing: {analyticsData.overview.avg_processing_time}
                </div>
              </div>

              <div className="bg-gradient-to-r from-purple-50 to-purple-100 p-2 border-2 border-purple-300 rounded">
                <div className="bold-title text-purple-800 font-bold text-xs uppercase">
                  Active Feeds
                </div>
                <div className="text-lg font-black text-purple-900">
                  {analyticsData.overview.active_rss_feeds}
                </div>
                <div className="italic-content text-purple-600 text-xs">
                  Success:{" "}
                  {
                    Object.values(analyticsData.source_analytics)[0]
                      ?.success_rate
                  }
                </div>
              </div>
          ...
          {showAnalytics && analyticsData && (
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 p-3 border-2 border-gray-300 rounded">
                <h3 className="bold-title text-base font-black text-black mb-2">
                  ⚡ PERFORMANCE METRICS
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <div className="bold-title text-sm font-bold text-gray-700">
                      API Response Times
                    </div>
                    <div className="space-y-1 text-sm">
                      <div className="italic-content">
                        50th percentile:{" "}
                        <span className="font-bold text-green-600">
                          {
                            analyticsData.performance_metrics.api_response_times
                              .p50
                          }
                        </span>
                      </div>
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="italic-content">
                        ...
                        <span className="font-bold text-blue-600">
                          {
                            analyticsData.performance_metrics.throughput
                              .articles_per_minute
                          }
                        </span>
                      </div>
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="italic-content">
                        ...
                        <span className="font-bold text-orange-600">
                          {
                            analyticsData.performance_metrics.system_resources
                              .cpu_usage
                          }
                        </span>
                      </div>
              ...
              <div className="bg-gray-50 p-3 border-2 border-gray-300 rounded">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  {Object.entries(analyticsData.source_analytics).map(
                    ([source, data]) => (
                      <div
                        key={source}
                        className="bg-white p-4 border border-gray-200 rounded"
                      >
                        <div className="bold-title font-bold text-black mb-2">
                          {source}
                        </div>
              ...
              <div className="bg-gray-50 p-3 border-2 border-gray-300 rounded">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div>
                    <div className="bold-title text-sm font-bold text-gray-700 mb-2">
                      Category Distribution
                    </div>
                    <div className="space-y-1 text-sm">
                      {Object.entries(
                        analyticsData.content_analysis.categories_distribution
                      )
                        .slice(0, 6)
                        .map(([category, count]) => (
                          <div
                            key={category}
                            className="italic-content flex justify-between"
                          >
                            <span>{category}:</span>
                            <span className="font-bold text-blue-600">
                              {count}%
                  ...
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="italic-content flex justify-between">
                        ...
                        <span className="font-bold text-green-600">
                          {
                            analyticsData.content_analysis.sentiment_analysis
                              .positive
                          }
                        </span>
                      </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="italic-content flex justify-between">
                        ...
                        <span className="font-bold text-green-600">
                          {
                            analyticsData.content_analysis.ai_confidence_scores
                              .high_confidence
                          }
                        </span>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
                  <div className="bg-white p-4 border border-green-200 rounded">
                    <div className="text-2xl font-black text-green-600">
                      {analyticsData.trend_analysis.growth_metrics.daily_growth}
                    </div>
                    <div className="bold-title text-sm font-bold text-gray-700">
                      Daily Growth
                    </div>
                  ...
                  <div className="bg-white p-4 border border-blue-200 rounded">
                    <div className="text-2xl font-black text-blue-600">
                      {
                        analyticsData.trend_analysis.growth_metrics
                          .weekly_growth
                      }
                    </div>
                    <div className="bold-title text-sm font-bold text-gray-700">
        ...
        <div className="newspaper-section mb-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              <div className="flex items-center space-x-4">
                <label className="bold-title font-black text-black uppercase">
                  FILTER BY CATEGORY:
                </label>
                <select
                ...
                >
                  ...
                  {statistics &&
                    statistics.categoryCounts &&
                    Object.keys(statistics.categoryCounts).map((category) => (
        ...
        {statusMessage && (
          <div
            className={`newspaper-section mb-3 ${
              isSuccessStatus
          ...
          >
            <div
              className={`font-bold text-lg ${
                isSuccessStatus ? "text-green-800" : "text-red-800"
              }`}
            >
        ...
        <div className="newspaper-section">
          ...
          {articles.length === 0 ? (
            <div className="text-center py-12">
              <div className="italic-content text-2xl font-bold text-gray-600 mb-4">
                No articles found
              </div>
              <button
                onClick={() => fetchNews()}
              ...
              {articles.map((article, index) => (
                <div key={article.id || index} className="newspaper-article">
                  <div className="article-header mb-3">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span
                        className="px-2 py-1 text-xs font-black uppercase tracking-wide text-white"
                        style={{
                          backgroundColor: getValidation(article).color,
                        }}
                      >
                  ...
                  <div className="article-content">
                    ...
                    <p className="italic-content text-gray-800 text-sm leading-relaxed mb-3">
                      {article.content}
                    </p>
                            ...
                            if (article.url) {
                              try {
                                ...
                                const response = await fetch(
                                  `http://localhost:8000/api/v1/news/scrape-article?url=${encodeURIComponent(
                                    article.url
                                  )}`
                                );
                                ...
                                if (response.ok) {
                                  ...
                                  openReaderMode({
                                    ...
                                    content:
                                      scrapedArticle.content || article.content,
                                    source:
                                      scrapedArticle.source || article.source,
                                    published_date:
                                      scrapedArticle.published_date ||
                                  ...
                                  console.warn(
                                    "Scraping failed, using original content"
                                  );
                                  openReaderMode({
                                    title: article.title,
        ...
        {statistics &&
          statistics.categoryCounts &&
          Object.keys(statistics.categoryCounts).length > 0 && (
            <div className="newspaper-section mb-2">
              ...
              <div className="newspaper-columns">
                {Object.entries(statistics.categoryCounts)
                  ...
                  .map(([category, count]) => {
                    const percentage =
                      ((count as number) / statistics.totalArticles) * 100;
                    return (
                      <div key={category} className="newspaper-article">
                        <div className="flex items-center justify-between mb-1">
                          <span className="bold-title font-black text-black uppercase text-xs">
                            {category}
                          </span>
                          <span className="italic-content font-bold text-black text-xs">
                            {count as number} articles
                          </span>
                        </div>
                        <div className="text-xs italic-content mb-1">
                          {percentage.toFixed(1)}% of total coverage
                        </div>
