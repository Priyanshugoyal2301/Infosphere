 ...
 */
async function fetchFromNewsData(category: string = 'top', limit: number = 50): Promise<DirectNewsArticle[]> {
  try {
 ...
 */
function normalizeCategory(category: string): { newsapi: string; gnews: string; newsdata: string } {
  const categoryMap: { [key: string]: { newsapi: string; gnews: string; newsdata: string } } = {
    ...
    'technology': { newsapi: 'technology', gnews: 'technology', newsdata: 'technology' },
    'entertainment': { newsapi: 'entertainment', gnews: 'entertainment', newsdata: 'entertainment' },
 ...
 */
export async function fetchDirectNews(category: string = 'all', limit: number = 150): Promise<DirectNewsArticle[]> {
  ...
  console.log(`✅ Fetched ${uniqueArticles.length} unique articles from all sources`);
 ...
 */
export async function searchDirectNews(query: string, limit: number = 100): Promise<DirectNewsArticle[]> {
  try {
    console.log(`🔍 Searching for: "${query}"`);
    
    // For search, we use NewsAPI's everything endpoint which supports search
    const url = `https://newsapi.org/v2/everything?q=${encodeURIComponent(query)}&pageSize=${limit}&sortBy=publishedAt&apiKey=${NEWS_API_KEY}`;
